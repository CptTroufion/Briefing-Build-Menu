...
function BBM:get_equipped_weapon(category)
...
local function build_part_data(weapon, part_type, raw_part, equipped_part, default_part, cosmetic_part)
	...
	local amount = (is_default or no_consume) and 1 or managers.blackmarket:get_item_amount(global_value, "weapon_mods", part_id, true)
...
function BBM:get_weapon_part_types(weapon)
	...
	return part_types, mods_by_type
end

function BBM:get_weapon_parts(weapon, part_type)
	local mods_by_type = managers.blackmarket:get_dropable_mods_by_weapon_id(weapon.crafted.weapon_id, {
		category = weapon.category,
		slot = weapon.slot
	})
	...
	for _, raw_part in ipairs(mods_by_type and mods_by_type[part_type] or {}) do
		local part = build_part_data(weapon, part_type, raw_part, equipped_part, default_part, cosmetic_part)
...
function BBM:get_weapon_modification_data(category)
	...
	local part_types = self:get_weapon_part_types(weapon)
	local data = { weapon = weapon, part_types = part_types, parts = {} }

	for _, part_type in ipairs(part_types) do
		data.parts[part_type] = self:get_weapon_parts(weapon, part_type)
	end

	return data
end

function BBM:confirm_install_weapon_part(category, part_type, part)
	local weapon = self:get_equipped_weapon(category)
	...
	local params = {
		...
		removes = removes or {},
		yes_func = function()
			self:install_weapon_part(category, part_type, part)
		end,
...
function BBM:install_weapon_part(category, part_type, part)
	...
	managers.blackmarket:buy_and_modify_weapon(category, weapon.slot, part.global_value, part.id, false, current_part.no_consume)
	...
	if not installed then
		self:show_error("bbm_weapon_transaction_error", part.id)
		self:refresh_weapon_modifications_gui()
		return
	end
	...
	if factory_tweak and factory_tweak.texture_switch then
		local default_texture = tweak_data.gui.part_texture_switches[part.id] or tweak_data.gui.default_part_texture_switch
		managers.blackmarket:set_part_texture_switch(category, weapon.slot, part.id, default_texture)
	end

	self:sync_player_outfit_while_open()
	self:refresh_weapon_modifications_gui()
end

function BBM:confirm_remove_weapon_part(category, part_type, part)
	local weapon = self:get_equipped_weapon(category)
	...
	local params = {
		...
		removes = removes or {},
		yes_func = function()
			self:remove_weapon_part(category, part_type, part)
		end,
		no_func = function() end
	}
...
function BBM:remove_weapon_part(category, part_type, part)
	if type(part) ~= "table" or not part.id then
		return
	end
	...
	if part.default_part then
		...
		local global_value = default_tweak and (default_tweak.infamous and "infamous" or default_tweak.dlc) or "normal"
		managers.blackmarket:buy_and_modify_weapon(category, weapon.slot, global_value, part.default_part, true, true)
	else
	...
	local default_restored = not part.default_part or weapon and table.contains(weapon.crafted.blueprint or {}, part.default_part)
	local succeeded = removed and default_restored
...
function BBM:refresh_weapon_modifications_gui()
	local gui = managers.menu_component and managers.menu_component._bbm_weapon_modifications
